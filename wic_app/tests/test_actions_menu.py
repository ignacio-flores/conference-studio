...
class ActionsMenuTests(unittest.TestCase):
    def test_actions_menu_exposes_public_bundle_publish_action(self) -> None:
        ...
        self.assertIn("export_presenter_roster_excel: Callable", text)
        ...
        self.assertIn("Show paper links in public exports and publish Excel", text)
        self.assertIn('public_show_links = st.checkbox(', text)
        ...
        self.assertIn("xlsx_path = export_publish_excel(", text)
        self.assertIn("presenters_path = export_presenter_roster_excel(state)", text)
        self.assertIn("show_links=public_show_links", text)
        self.assertIn("pdf_path = export_publish_pdf(state, publish_display=publish_display)", text)
        self.assertIn("Publish Word:", text)
        self.assertIn("Presenter roster:", text)
        self.assertIn("bundle_path = publish_public_bundle(", text)
        self.assertIn("publish_display=publish_display", text)
        self.assertIn("preview_details = preview_public_bundle(", text)
    ...
    def test_app_wires_public_bundle_publish_action_into_menu(self) -> None:
        ...
        self.assertIn("export_presenter_roster_excel=export_presenter_roster_excel", text)
        self.assertIn("export_publish_docx=export_publish_docx", text)
        self.assertIn('publish_display: str = "full"', text)
        self.assertIn("show_links: bool = True", text)
